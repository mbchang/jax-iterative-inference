...
encoder_init, encode = stax.serial(
    stax.parallel(
        ...
        stax.serial(stax.FanInConcat(), Dense(256), Relu)  # mu, logvar, gradmu, gradlogvar
    ),
    stax.FanInConcat(),
    Dense(512), Relu,
    FanOut(2),
...
def outer_loss(rng, params, images, num_steps, return_images=False):
    ...
    prior_mu_z = jnp.repeat(jnp.expand_dims(prior_mu_z, 0), repeats=images.shape[0], axis=0) # (B, D)
    prior_logvar_z = jnp.repeat(jnp.expand_dims(prior_logvar_z, 0), repeats=images.shape[0], axis=0) # (B, D)
    ...
    for t in range(num_steps):
        ...
        posterior_mu_z, posterior_logvar_z = encode(enc_params,
                                                    (images, (z_dist.mu, z_dist.logvar, elbo_grad.mu, elbo_grad.logvar)))
    ...
    to_return = (loss/num_steps, logit_images, logvar_z_norms, elbos) if return_images else loss/num_steps
    return to_return
...
def main():
    ...
    _, init_encoder_params = encoder_init(
        enc_init_rng, ((batch_size, 28*28), [(batch_size, latent_dim) for i in range(4)]))
    ...
    def evaluate(opt_state, images):
        ...
        test_elbo, logit_images, logvar_z_norms, elbos = outer_loss(elbo_rng, params,
                                binarized_test, num_steps, True) 
        stack_logit_images = jnp.stack(logit_images)
        sampled_logit_images = image_grid(num_steps+1, num_test, stack_logit_images, (28, 28))
        return -test_elbo/images.shape[0], sampled_logit_images, logvar_z_norms, elbos
    ...
    for epoch in range(num_epochs):
        ...
        if not debug:
            ...
            test_elbo, logit_images, logvar_norms, elbos = jit(evaluate)(opt_state, test_images)
        else:
            rng = random.PRNGKey(epoch)
            for i in range(num_batches):
                elbo_rng, data_rng = random.split(random.fold_in(rng, i))
                batch = binarize_batch(data_rng, i, train_images)
                loss = lambda params: -outer_loss(elbo_rng, params, batch, num_steps) / batch_size
